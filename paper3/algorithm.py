...
def single_multiprocessing(args,percent):
    ...
    for loc in loc_of_first_feature:
        ...
        if max_k_aac > max_loc_aac:
            ...
            print(">: {}\n".format(max_aac_list))
            with lock:
                infor_file = open("{}/{}_outpot.txt".format(percent,dataset_filename.split(".")[0]),"a")
                infor_file.write(">: {}\n".format(max_aac_list))
                infor_file.close()
...
def test_dataset_multiprocessing():
    ...
    single_dataset= partial(single,[dataset_filename,label_filename,lock])

    pool = multiprocessing.Pool(3)
    lock = multiprocessing.Manager().Lock()
    result = pool.map(single_dataset,percents)
