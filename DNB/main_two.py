...
def prepare(option_dict):
    ...
    for i in range(option_dict["periods"]):
        ...
        dataset_list.append(fdr_t_value_filter(case_dataset_list[i],control_dataset_list[i],fdr = option_dict["fdr"]))
    print("\n")
...
def normalize_all(all_periods_case_clusters,control_dataset):
    ...
    for cluster_list in all_periods_case_clusters:
        normalize_cluster.append([normalize(cluster,control_dataset) for cluster in cluster_list])
...
def calculate_outer_pcc(normalize_cluster):
    pearsons = []
    for gene_id_inner, value_inner in normalize_cluster.iterrows():
        for gene_id_outer, value_outer in normalize_cluster.iterrows():
...
def calculate_ci(normalize_cluster):
    #print(normalize_cluster)
    inner_pear = calculate_inner_pcc(normalize_cluster)
    outer_pear = calculate_outer_pcc(normalize_cluster)
...
def work(cluster_list):
    case_cis = []
    for cluster in cluster_list:
        case_cis.append(calculate_ci(cluster))
    return case_cis
...
def multiprocessing_clusetr(normalize_cluster_list):
    pool = multiprocessing.Pool(multiprocessing.cpu_count())
    result = pool.map(work,normalize_cluster_list)
...
def main():
    ...
    #准备不同时间片的数据
    case_dataset_list, raw_case_dataset_list, control_dataset_list  = prepare(option_dict)
    ...
    for i in range(len(case_dataset_list)):
        ...
        #得到每个类在不同的时间片内的所有group
        all_periods_case_clusters = get_all_periods_clusters(all_case_clusters,raw_case_dataset_list)
        
        #进行 normalize
        normalize_cluster_list = normalize_all(all_periods_case_clusters,control_dataset_list[i])
        
        #result = work(normalize_cluster_list[0])
        #print(result)

        result = multiprocessing_clusetr(normalize_cluster_list)
        print("\n")
