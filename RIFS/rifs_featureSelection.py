...
def t_test(dataset, labels):
    ...
    t_value, p_value = ttest_ind_from_stats(
        p_mean, p_std, p_feature_data.shape[1], n_mean, n_std, n_feature_data.shape[1])
...
#----------------------------------
    ...
    for loc in loc_of_first_feature:
        ...
        if max_k_aac > max_loc_aac:
            ...
            infor_file = open(
                "{}/{}_result.txt".format(seed_number, dataset_filename.split(".")[0]), "a")
            infor_file.write(">: {}\n".format(max_aac_list))
            infor_file.close()
    ...
    infor_file = open("{}/{}_result.txt".format(seed_number,
                                                dataset_filename.split(".")[0]), "a")
