...
def load_data(filename):
    ...
    with open("{}_index.pkl".format(filename.split(".")[0]),"wb") as f:
...
#----------------------------------    
    ...
    if not os.path.exists("{}".format(dataset_filename.split(".")[0])):
        os.mkdir("{}".format(dataset_filename.split(".")[0]))
    ...
    for loc in loc_of_first_feature:
        ...
        if max_k_aac > max_loc_aac:
            ...
            infor_file = open("{}/{}.txt".format(dataset_filename.split(".")[0],stop),"a")
            infor_file.write(">: {}\n".format(max_aac_list))
            infor_file.close()
...
def all_dataset():
    ...
    for dataset_filename,labels_filename in zip(sorted(dataset_list),sorted(label_list)):
        for stop_number in stop_list:
            single(dataset_filename,labels_filename,stop = stop_number)
