...
def read_csv_file(p_filename,n_filename):
    ...
    p_dataset = pd.read_csv(p_filename,index_col = 0)
    n_dataset = pd.read_csv(n_filename, index_col = 0)
    dataset = np.vstack((p_dataset,n_dataset))   #[sample, feature]
...
def evaluate(estimator,X,y,skf):
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        Tn = float(sum([1 for x in list(y_true[predict_index_n]) if x == 0])) #正确预测的负类   (实际为负类 预测为负类)
...
#-------------参数------------------    
    n = 10    #采用 n 折交叉验证
    p_filename = "Normal_sub.csv"
    n_filename = "Gastric_polyp_sub.csv"
#-------------参数------------------       
