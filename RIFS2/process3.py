...
def recursive_elimination(datasets_dict,labels_dict,feature_list,acc_list):
    ...
    while True:
        ...
        if len(eliminate_feature) == 0:
            pass 
        elif len(set(eliminate_feature)) == 1:
            feature_list.remove(eliminate_feature[0])
            print("\nremoveing feature {}".format(eliminate_feature[0]))
        else:   
            print("\n")
            for _ in eliminate_feature:
                feature_list.remove(_) 
                print("removeing feature {}".format(_))
