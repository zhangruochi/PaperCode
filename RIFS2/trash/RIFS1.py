...
#对每一个数据集进行运算
def single(dataset_filename,clinical_filename, threshold, criterion ,lock = None):
#------------参数接口---------------    
...
def generate_features(dataset_filename,clinical_filename,threshold):
    criterion_list = [[[1],[2]],[[1],[3]],[[2],[3]],[[1,2],[3]],[[1,3],[2]],[[2,3],[1]]]
    jobs = []
    for i in range(6):
        process = multiprocessing.Process(target=single, args=(dataset_filename,clinical_filename,threshold, criterion_list[i], Lock(),))
        jobs.append(process) 
...
if __name__ == '__main__':
    ...
    #feature_names = generate_features(dataset_filename = "LUSC/matrix_data_lusc.tsv",clinical_filename = "LUSC/clinical_LUSC.json",threshold = 0.7)
    process_result()
