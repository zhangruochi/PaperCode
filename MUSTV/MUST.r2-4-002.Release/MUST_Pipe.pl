...
system("makeblastdb -p F -i \"$egFileMITESeq\"");
...
if(system("cp \"$egFileSeq\" \"$tFileSeq\"")or system("makeblastdb -p F -i \"$tFileSeq\"")or system("blat \"$tFileSeq\" \"$tFileTemp\" -minIdentity=95 -minScore=100 \"$tFileBLAT\" 2>&1 1>>\"$egDirTemp/temp-genome-seq.blat.log\" ")){die"-Error when mapping!\n";}print" [BLAT]";
my@pGID=();
my@pS=();
my@pE=();
my@pStrand=();
