...
%%
%   以下是自己对这个代码的分析状况:
%   mapping这个文件时用来提取纹理特征的 同时适用于LBP和LCP只不过LCP多一个MIC的过程而已
%   riu2是统一的旋转不变性 samples是其在附近采集的样本点的个数，使用该函数之后， 返回的是一个表格 
%   一般用mapping这个变量来将其记录下来就行了
%   具体的使用方法是 “ mapping=getmapping（n ,'riu2')                                                                                       ” 
%   n一般为 8 10 12 (半径对应的是2 3 4) -> 所提取的特征维数一般为8 121 169 ; riu2一般都加上。
%%
