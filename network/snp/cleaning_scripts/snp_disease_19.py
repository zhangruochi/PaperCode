...
def add_tag_group_Substitutions(row,tag_group_dict,other_tag_group_dict):
    ...
    for name,attrs in tag_group_dict.items():
        if name in row["disease"]:
            return [row["genomic_coordinates_hg19"],name,line[0],int(line[1]),int(line[1]),attrs[0],attrs[1]]

    for name,attrs in other_tag_group_dict.items():
        for _ in name.split(" "):
            if _ in row["disease"]:
                return [row["genomic_coordinates_hg19"],_,line[0],int(line[1]),int(line[1]),attrs[0],attrs[1]]
...
def add_tag_group_Lesions(row,tag_group_dict,other_tag_group_dict):
    ...
    for name,attrs in tag_group_dict.items():
        if name in row["disease"]:
            return [row["genomic_coordinates_hg19"],name,line[0],int(pos_start),int(pos_end),attrs[0],attrs[1]]

    for name,attrs in other_tag_group_dict.items():
        for _ in name.split(" "):
            if _ in row["disease"]:
                return [row["genomic_coordinates_hg19"],_,line[0],int(pos_start),int(pos_end),attrs[0],attrs[1]]
...
def process_HGMD(database_file):
    ...
    dataset = dataset[["Variant_class","disease","genomic_coordinates_hg19","sequence_context_hg19"]].dropna()
    #print(dataset);exit()
    ...
    if "Substitutions" in database_file:
        dataset.loc[:,["genomic_coordinates_hg19","disease","#CHROM","POS","POS_END","HITag","Group"]] = dataset.loc[:,["genomic_coordinates_hg19","disease","#CHROM","POS","POS_END","HITag","Group"]].apply(func = add_tag_group_Substitutions,axis = 1, args = (tag_group_dict,other_tag_group_dict,))
    elif "Lesions" in database_file:
        dataset.loc[:,["genomic_coordinates_hg19","disease","#CHROM","POS","POS_END","HITag","Group"]] = dataset.loc[:,["genomic_coordinates_hg19","disease","#CHROM","POS","POS_END","HITag","Group"]].apply(func = add_tag_group_Lesions,axis = 1, args = (tag_group_dict,other_tag_group_dict,))
